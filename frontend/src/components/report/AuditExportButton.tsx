...
const WEBAPP_URL = 'https://linuxaioperf.com';

function escapeHtml(value: unknown) {
  return String(value ?? '—').replace(/[&<>'"]/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[char]!));
}
function gib(value?: number) { return value === undefined ? '—' : `${(value / 1024 ** 3).toFixed(2)} GiB`; }
function noteHtml(notes: Notes, section: string) { return notes[section]?.trim() ? `<p class="note"><b>Auditor note</b>${escapeHtml(notes[section].trim())}</p>` : ''; }
...
export default function AuditExportButton({ data }: { data: ReportData }) {
  ...
  return <div className="flex flex-col items-end gap-2"><div className="flex gap-2"><button onClick={() => setNotesOpen(!notesOpen)} className="px-3 py-2 rounded-lg text-sm" style={{ background:'var(--bg-elevated)', border:'1px solid var(--border)', color:'var(--text-primary)' }}>{notesOpen ? 'Hide notes' : 'Add audit notes'}</button><button onClick={exportAudit} disabled={busy} className="px-3 py-2 rounded-lg text-sm font-medium disabled:opacity-60" style={{ background:'var(--accent)', color:'var(--bg-base)' }}>{busy ? 'Preparing audit…' : 'Export audit PDF'}</button></div>{notesOpen && <div className="w-full grid gap-2 md:grid-cols-2">{NOTE_SECTIONS.map(section => <label key={section} className="text-xs text-left" style={{ color:'var(--text-secondary)' }}>{section}<textarea value={notes[section] ?? ''} onChange={event => setNotes({ ...notes, [section]: event.target.value })} rows={2} className="mt-1 w-full rounded p-2 text-sm" style={{ background:'var(--bg-elevated)', border:'1px solid var(--border)', color:'var(--text-primary)' }} placeholder="Optional note for the PDF…" /></label>)}</div>}{error && <span className="text-xs text-red-400">{error}</span>}</div>;
}
