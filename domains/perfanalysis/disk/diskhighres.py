...
class DiskHighResProcessor(BaseDataProcessor):
    ...
    def extract_header(self) -> str:
        ...
        return "Timestamp Major Minor Device Reads_Completed Reads_Merged Sectors_Read Time_Reading Writes_Completed Writes_Merged Sectors_Written Time_Writing IO_Currently IO_Time Weighted_IO_Time Discards_Completed Discards_Merged Sectors_Discarded Time_Discarding Flush_Requests Time_Flushing"
    ...
    def process_data(self) -> pd.DataFrame:
        ...
        try:
            columns = [
                ...
                'IO_Currently', 'IO_Time', 'Weighted_IO_Time',
                'Discards_Completed', 'Discards_Merged', 'Sectors_Discarded',
                'Time_Discarding', 'Flush_Requests', 'Time_Flushing'
            ]
            
            df = pd.read_csv(self.input_file, sep=r'\s+', names=columns, skiprows=1)
    ...
    def _calculate_rates(self, device_data: pd.DataFrame, device_name: str) -> pd.DataFrame:
        """
    ...
    def _resample_device_data(self, device_data: pd.DataFrame, resample_period: str = '1s') -> pd.DataFrame:
        """
    ...
    def create_plots(self, df: pd.DataFrame) -> List[go.Figure]:
        ...
        try:
            if df.empty:
                self.logger.warning("No data available for high-resolution disk plots")
                return []
            ...
            fig_throughput.update_traces(line={'shape': 'linear'}, mode='lines')
                ...
                updatemenus=[dict(
                    ...
                    buttons=[
                        dict(
                            ...
                            method="update",
                            args=[{
                                "visible": [False, True, False, True] * len(devices)
                            }]
                        ),
                        dict(
                            label="50ms",
                            method="update",
                            args=[{
                                "visible": [True, False, True, False] * len(devices)
                            }]
                        )
                    ]
                )],
                ...
                updatemenus=[dict(
                    ...
                    buttons=[
                        dict(
                            ...
                            method="update",
                            args=[{
                                "visible": [True, True, False, False] * len(devices)
                            }]
                        ),
                        dict(
                            label="1s avg",
                            method="update",
                            args=[{
                                "visible": [False, False, True, True] * len(devices)
                            }]
                        )
                    ]
                )]
            ...
            raise DataProcessorError(f"Failed to create high-resolution disk plots: {e}")
