...
import { buildAuditHeader, type AuditExportOptions } from './auditExportOptions';
...
function printDocument(data: ReportData, charts: ChartImage[], notes: Notes, options: AuditExportOptions) {
  ...
  const headerHtml = buildAuditHeader(options, logo);
  const browserTitle = escapeHtml(options.title.trim() || 'Linux AiO – Performance Report');
  const contextItems = [
    ['Hostname', meta.hostname], ['Report started', meta.capture_start], ['Report finished', meta.capture_end],
    ['Collection runtime', meta.runtime], ['Operating system', meta.os], ['Kernel', meta.kernel],
    ['CPU model', meta.cpu_model], ['Report ID', data.report_id],
  ].filter(([, value]) => Boolean(value));
  const contextHtml = `<div class="context">${contextItems.map(([label, value]) => `<div><b>${escapeHtml(label)}</b><span>${escapeHtml(value)}</span></div>`).join('')}</div>`;
  ...
  popup.document.write(`<!doctype html><html><head><title>${browserTitle}</title><style>
  ...
  </style></head><body><button class="no-print" onclick="window.print()">Print / Save as PDF</button>${headerHtml}<section><h2>Host context</h2>${noteHtml(notes, 'Host context')}${contextHtml}</section>${healthHtml}${storageHtml(data, notes)}${chartHtml(charts, notes)}<p class="footer"><a href="https://linuxaioperf.matildes.dev">https://linuxaioperf.matildes.dev</a> | <a href="https://github.com/samatild/LinuxAiOPerf">github.com/samatild/LinuxAiOPerf</a></p></body></html>`);
...
export default function AuditExportButton({ data }: { data: ReportData }) {
  ...
  const [options, setOptions] = useState<AuditExportOptions>({
    title: 'Linux AiO – Performance Report',
    caseId: '',
    includeLogo: true,
  });
  const exportAudit = async () => {
    ...
    try {
      ...
      printDocument(data, charts, notes, options);
  ...
  return <>
    ...
    {createPortal(<><button onClick={() => setNotesOpen(true)} aria-label="Open notes" className="report-notes-tab">Notes</button>{notesOpen && <div className="fixed inset-0 z-[110]" role="dialog" aria-modal="true" aria-label="Report notes"><button className="absolute inset-0 w-full h-full" aria-label="Close notes" onClick={() => setNotesOpen(false)} style={{ background:'rgba(0,0,0,.45)' }} /><aside className="absolute right-0 top-0 h-full w-full max-w-md overflow-y-auto p-5" style={{ background:'var(--bg-surface)', borderLeft:'1px solid var(--border)' }}><div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold">Notes</h2><button onClick={() => setNotesOpen(false)} className="px-2 py-1 rounded" style={{ color:'var(--text-secondary)' }}>Close</button></div><p className="text-sm mb-4" style={{ color:'var(--text-secondary)' }}>Optional notes are included only in their matching PDF section.</p><fieldset className="grid gap-3 mb-5"><legend className="text-sm font-medium mb-2">PDF details</legend><label className="text-sm" style={{ color:'var(--text-secondary)' }}>Title override<input value={options.title} onChange={event => setOptions({ ...options, title: event.target.value })} className="mt-1 w-full rounded p-2 text-sm" style={{ background:'var(--bg-elevated)', border:'1px solid var(--border)', color:'var(--text-primary)' }} /></label><label className="text-sm" style={{ color:'var(--text-secondary)' }}>Ticket / Support case number<input value={options.caseId} onChange={event => setOptions({ ...options, caseId: event.target.value })} className="mt-1 w-full rounded p-2 text-sm" style={{ background:'var(--bg-elevated)', border:'1px solid var(--border)', color:'var(--text-primary)' }} placeholder="Optional" /></label><label className="flex items-center gap-2 text-sm" style={{ color:'var(--text-secondary)' }}><input type="checkbox" checked={options.includeLogo} onChange={event => setOptions({ ...options, includeLogo: event.target.checked })} />Include LinuxAiO logo</label></fieldset><div className="grid gap-3">{NOTE_SECTIONS.map(section => <label key={section} className="text-sm" style={{ color:'var(--text-secondary)' }}>{section}<textarea value={notes[section] ?? ''} onChange={event => setNotes({ ...notes, [section]: event.target.value })} rows={3} className="mt-1 w-full rounded p-2 text-sm" style={{ background:'var(--bg-elevated)', border:'1px solid var(--border)', color:'var(--text-primary)' }} placeholder="Optional note for this section…" /></label>)}</div></aside></div>}</>, document.body)}
