...
class TemplateManager:
    ...
    def inject_static_resources(self, content: str) -> str:
        ...
        css_injection = (
            ...
            f'href="{self.static_url}">')
        content = content.replace(
            "<!-- header.script1_placeholder -->", css_injection)

        # Inject JavaScript - this should be injected after the font-awesome
        # link
        js_injection = (
            f'<script src="{self.js_static_url}" type="text/javascript">'
            '</script>')
        content = content.replace(
            "<!-- header.script2_placeholder -->", js_injection)
    ...
    def inject_version_info(self, content: str, version: str) -> str:
        ...
        content = content.replace(
            "<!-- footer.version_placeholder -->", version_info)
