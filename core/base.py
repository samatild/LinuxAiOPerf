...
class BaseDataProcessor(ABC):
    ...
    def __init__(self, input_file: str, output_dir: str = ".", logger: Optional[logging.Logger] = None):
        """
    ...
    def _validate_input_file(self) -> None:
        ...
        if not os.access(self.input_file, os.R_OK):
            raise DataProcessorError(f"Cannot read input file: {self.input_file}")
        
        if os.path.getsize(self.input_file) == 0:
            raise DataValidationError(f"Input file is empty: {self.input_file}")
    ...
    def write_filtered_data(self, output_file: str, header: str, data_lines: List[str]) -> None:
        """
    ...
    def get_common_plot_layout(self, title: str, x_title: str = "Timestamp", y_title: str = "Value") -> Dict[str, Any]:
        """
    ...
    def process(self) -> Tuple[pd.DataFrame, List[go.Figure]]:
        ...
        try:
            ...
            raise DataProcessorError(f"Failed to process {self.input_file}: {e}")
...
class SystemInfoProcessor(BaseDataProcessor):
    ...
    def process(self) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"Starting system info processing of {self.input_file}")
            return self.extract_system_info()
        except Exception as e:
            self.logger.error(f"System info processing failed: {e}")
            raise DataProcessorError(f"Failed to process system info {self.input_file}: {e}")
...
class ProcessInfoProcessor(BaseDataProcessor):
    ...
    def process(self) -> Tuple[Dict[str, str], set, str]:
        ...
        """
        try:
            self.logger.info(f"Starting process info processing of {self.input_file}")
            return self.extract_process_data()
        except Exception as e:
            self.logger.error(f"Process info processing failed: {e}")
            raise DataProcessorError(f"Failed to process process info {self.input_file}: {e}")
